...
def scrape_tech_links(base_url, keywords, max_links=500):
    links = []
    ...
    try:
        while pages_to_scrape and len(links) < max_links:
            ...
            for a_tag in soup.find_all('a', href=True):
                ...
                if any(keyword in href.lower() for keyword in keywords) or any(keyword in lower_title for keyword in keywords):
...
def insert_into_snowflake_bulk(links):
    try:
        ...
        ''')
        logging.info("Main table TECH_LINKS created or already exists with AUTO_INCREMENT.")
        ...
        # Skip IDs because Snowflake will handle them
        cursor.executemany('''
        ...
        logging.error(f"Error occurred while inserting into Snowflake: {e}")
        raise
...
if __name__ == "__main__":
    ...
    # Insert the collected links, titles, id, and timestamp into Snowflake
    if tech_links:
